...
export async function generateMetadata({ params }: { params: { navbarcategory: string, category: string } }): Promise<Metadata> {
  try {
    ...
    if (!params.navbarcategory || params.navbarcategory === 'undefined' || 
        !params.category || params.category === 'undefined') {
      console.error('Invalid category params:', params);
      return {
    ...
    const category = await getCategory(params.navbarcategory, params.category);
    ...
    return {
      ...
      description: 'Error loading category information.'
    };
  }
}

async function getCategory(navbarCategorySlug: string, categorySlug: string) {
  try {
    // Validate slugs
    if (!navbarCategorySlug || navbarCategorySlug === 'undefined' || 
        !categorySlug || categorySlug === 'undefined') {
      console.error('Invalid slugs provided:', { navbarCategorySlug, categorySlug });
      return null;
    }
    ...
    if (!category) {
      console.error(`Category with slug "${categorySlug}" not found. Check the database for existing categories.`);
      return null;
    }

    // Verify the navbar category matches
    if (category.navbarCategoryId?.slug !== navbarCategorySlug) {
      console.error(`Navbar category mismatch. Expected: ${navbarCategorySlug}, Got: ${category.navbarCategoryId?.slug}`);
      return null;
    }
