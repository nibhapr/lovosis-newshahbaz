...
export default function ProductPage({
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (hasChanged) {
          setProduct(data);
          setCatalogImages(data.catalogImages || [data.catalogImage].filter(Boolean));
        }
      } else {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && 'WebSocket' in window) {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (data.type === 'product-update' && data.productSlug === resolvedParams.product) {
              fetchProduct();
            }
          } catch (error) {
        ...
        ws.onerror = (error) => {
          ...
          if (!pollingInterval) {
            pollingInterval = setInterval(fetchProduct, 5000); // Poll every 5 seconds
          }
        };

        ws.onclose = (event) => {
          console.log('WebSocket connection closed:', event.code, event.reason);
          // Start polling when WebSocket closes
          if (!pollingInterval) {
            pollingInterval = setInterval(fetchProduct, 5000);
          }
        };
      } else {
      ...
      console.error('Error setting up WebSocket, using polling:', error);
      pollingInterval = setInterval(fetchProduct, 5000);
    }
  ...
  useEffect(() => {
    document.body.style.overflow = (isReviewsOpen || isContactFormOpen) ? 'hidden' : 'unset';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to create catalog request');
                ...
                {reviews.length > 0 ? (
                  <>
                    {reviews.slice(0, 2).map((review) => (
                      <div key={review._id} className="bg-zinc-800/50 p-3 rounded-lg">
        ...
        <>
          ...
          >
            ...
            >
              ...
              <div 
                className="h-full w-full relative cursor-zoom-in overflow-hidden"
              >
                <div 
                  className="h-full w-full transition-transform duration-100 origin-center"
                  style={{
                    transform: 'scale(1)',
                    backgroundImage: `url(${catalogImages[currentCatalogIndex]})`,
                  ...
                  onMouseMove={(e) => {
                    ...
                    const scrollX = (xPercent - 50) * 2;
                    const scrollY = (yPercent - 50) * 2;
