...
const Navbar = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      console.log('Starting to fetch navbar data...');
      ...
      if (!navbarResponse.ok) {
        ...
        console.error('Navbar categories API failed:', navbarResponse.status, errorText);
        throw new Error(`Failed to fetch navbar categories: ${navbarResponse.status} - ${errorText}`);
      }
      ...
      if (!subcategoriesResponse.ok) {
        ...
        console.error('Subcategories API failed:', subcategoriesResponse.status, errorText);
      ...
      console.log('Data fetched successfully:', {
        navbarCount: navbarData?.length || 0,
        categoriesCount: categoriesData?.length || 0,
        subcategoriesCount: subcategoriesData?.length || 0
      });

      // Format data efficiently with null checks
      ...
      console.log('Navbar data set successfully');
      
    } catch (error) {
      console.error('Error fetching navbar data:', error);
      
      // Set empty data instead of keeping loading state
      setNavbarCategories([]);
